...
impl DomainLayer {
    ...
    pub async fn new(infrastructure_layer: Arc<InfrastructureLayer>) -> Self {
        // 初始化用户子领域
        let user_domain = Arc::new(UserDomain::new(infrastructure_layer.persistence.repository.user_repository.clone()).await);

        // 初始化钱包子领域
        let wallet_domain = Arc::new(WalletDomain::new(infrastructure_layer.persistence.repository.wallet_repository.clone()).await);
